...
from utils import (
    ...
    get_sentiment_advice, draw_trading_chart, test_alert
)
import pandas as pd
...
# ========================================
# DASHBOARD (YOUR FAVORITE LOOK — FIXED)
# ========================================
with tab1:
    st.header("Add Stock")
    df = load_investments()
    ...
    if not df.empty:
        sel = st.selectbox("Choose:", df['ticker'])
        ...
        if err:
            ...
            vol = f"{data['volume']:,}"
            chg = data['change_pct']
            market = "Open" if data['market_open'] else "Closed"

        # YOUR FAVORITE METRICS
        c1, c2, c3, c4, c5 = st.columns(5)
        c1.metric("Set", "0")
        c2.metric("Breakeven", f"₹{r['buy_price']:.2f}")
        c3.metric("Stage", f"Profit: {pnl:+.1f}%" if not err else "—")
        sent, adv = get_sentiment_advice(sel)
        c4.metric("Sentiment", sent)
        c5.metric("Advice", adv)

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Current", f"₹{cur:.2f}" if not err else "—")
        c2.metric("High", f"₹{high:.2f}" if not err else "—")
        c3.metric("Low", f"₹{low:.2f}" if not err else "—")
        c4.metric("Volume", vol)

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Change", f"{chg:+.2f}%" if not err else "—")
        c2.metric("P&L", f"{pnl:+.1f}%" if not err else "—")
        c3.metric("Qty", r['qty'])
        c4.metric("Market", market)

        st.subheader(f"Chart & Forecast: {sel}")
        fig = draw_trading_chart(sel)
