...
# Import utils (which now has st inside — safe because Streamlit is already running)
from utils import (
...
def check_alerts(df_pnl: pd.DataFrame):
    ...
    for _, row in st.session_state.alerts.iterrows():
        ...
        if row['type'] == 'price' and cur >= row['target_price']:
            msg = f"{sym} hit target ${row['target_price']} → ${cur:.2f}"
            ...
            if pnl <= -abs(row['drop_pct']):
                msg = f"{sym} dropped {row['drop_pct']}% → {pnl:.1f}%"

        if msg:
            send_email(f"Stock Alert: {sym}", msg, st.session_state.user_email)
            send_whatsapp(msg)
...
st.caption("StockGuardian © 2025 | Built with Streamlit, yfinance, Prophet, Hugging Face")
